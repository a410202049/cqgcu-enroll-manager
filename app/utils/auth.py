...
class Auth(object):
    ...
    def auth_menus(self):
        ...
        menus = db.session.query(
            MenuAuth
        ).order_by('sort asc').filter(MenuAuth.type == 0).all()
        # menus = MenuAuth.query.order_by('sort asc').filter(MenuAuth.type == 0).all()
        # current_menus = MenuAuth.query.filter(MenuAuth.method == path).scalar()
